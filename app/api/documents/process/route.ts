  ...
  if (!process.env.GOOGLE_AI_API_KEY) {
    await supabase
      ...
      .update({
        ...
        error_message:
          "GOOGLE_AI_API_KEY not configured. The file is stored, but the AI pipeline is disabled until a key is set in .env.local.",
      })
      .eq("id", document_id);
    return NextResponse.json({ ok: true, skipped: true, reason: "google_ai_key_missing" });
  }
