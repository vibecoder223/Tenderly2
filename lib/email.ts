...
<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;color:#1a1a1a;">
  <div style="font-weight:700;font-size:18px;letter-spacing:-0.02em;margin-bottom:24px;">Propello</div>
  ${inner}
...
export function inviteEmail(opts: {
  ...
  return {
    subject: `Join ${opts.orgName} on Propello`,
    html: WRAP(`
...
export function resetEmail(opts: { resetUrl: string }): { subject: string; html: string; text: string } {
  return {
    subject: "Reset your Propello password",
    html: WRAP(`
    ...
    `),
    text: `Reset your Propello password:\n\n${opts.resetUrl}`,
  };
}
